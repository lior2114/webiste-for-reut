...
import { Container, Typography, Paper, Alert, TextField, Grid, Button, Box, CircularProgress } from '@mui/material';
...
import { getVacationById, updateVacation } from '../../api/api';
...
export function EditVacation() {
  ...
  React.useEffect(() => {
    const loadVacation = async () => {
      try {
        ...
        setForm({
          vacation_destination: vacation.vacation_destination || '',
          ...
          vacation_image: vacation.vacation_image || '',
        });
      } catch (e) {
        setError(e.message || t('טעינת החופשה נכשלה', 'Failed to load vacation'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await updateVacation(id, vacationData);
      navigate('/vacations');
    } catch (e) {
      setError(e.message || t('עדכון נכשל', 'Update failed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Container maxWidth="md">
      <Paper className={styles.paper}>
        <Typography variant="h4" gutterBottom>{t('עריכת חופשה', 'Edit Vacation')}</Typography>
